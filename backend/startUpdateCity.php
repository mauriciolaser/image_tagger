...
my_log("DEBUG: Iniciando startUpdateCity.php");

// Obtener el parámetro 'city' de la URL y validarlo
$cityParam = filter_input(INPUT_GET, 'city', FILTER_SANITIZE_STRING);
if (!$cityParam) {
    http_response_code(400);
    exit(json_encode([
        "success" => false,
        "message" => "Parámetro 'city' no válido"
    ]));
}
...
try {
    ...
    // Definir las llaves de prefijos para filtrar y asignar ciudades
    $prefixes = [
        'esp_bar_' => 'barcelona',
        'fin_tam_' => 'tampere',
        'fin_hel_' => 'helsinki',
        ...
        if (($handle = fopen($csvFile, 'r')) !== false) {
            ...
            if ($header !== false) {
                ...
                while (($row = fgetcsv($handle)) !== false) {
                    ...
                    // Filtrar solo archivos cuyo filename comience con el prefijo definido igual al parámetro recibido
                    $matched = false;
                    foreach ($prefixes as $prefix => $cityName) {
                        if (strpos($fileCsv, $prefix) === 0) {
                            // Solo marcar si el prefijo coincide exactamente con el valor recibido
                            if ($cityParam === $prefix) {
                                $matched = true;
                            }
                            break;
    ...
    $conn->rollback();
    error_log("Error en `startUpdateCity.php`: " . $e->getMessage());
    http_response_code(500);
